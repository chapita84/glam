...
import { Search, MapPin, Star, Clock, DollarSign, Calendar, User, Settings, Plus, Loader2 } from 'lucide-react';
import { getAllStudios, getServicesForStudio } from '@/lib/firebase/firestore';
import { Studio, Service } from '@/lib/types';
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { cleanSpecificStudio, deleteProblematicStudio } from '@/lib/cleanStudio';
...
export default function CustomerPage() {
  ...
  useEffect(() => {
    const fetchStudiosAndServices = async () => {
      ...
      try {
        ...
        console.log('Final studios with services:', studiosWithServices);
  ...
  const handleCleanData = async () => {
    ...
    try {
      // Primero limpiar el estudio problemático
      await cleanSpecificStudio();
      
      // Luego recargar los estudios
      const fetchStudiosAndServices = async () => {
        try {
          setLoading(true);
          console.log('Reloading studios after cleaning...');
          const allStudios = await getAllStudios();
          console.log('Studios reloaded:', allStudios);
          ...
          console.error('Error reloading studios:', error);
        } finally {
          setLoading(false);
        }
      };
      await fetchStudiosAndServices();
    } catch (error) {
            ...
            <div className="md:w-64">
              ...
              >
                ...
                {getAllServices().map((service, index) => (
                  <option key={service.id || `service-${index}-${service.name}`} value={service.name}>
                    {service.name}
                  </option>
                ))}
              </select>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {studio.services.slice(0, 3).map((service, index) => (
                            <div
                              key={service.id || `${studio.id}-service-${index}-${service.name}`}
                          ...
                          {studio.services.length > 3 && (
                            <p className="text-xs text-gray-500">
                              +{studio.services.length - 3} servicios más
                            </p>
                          )}
                        </div>
                      </div>
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">¡Hola, {profile.displayName || profile.email}! 👋</h1>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="cursor-pointer transition-transform hover:scale-105">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Editar información personal y preferencias
            </p>
