...
export default function ServicesPageClient() {
  const { services, studioId, refreshData, loading } = useStudioData();
  ...
  const handleSaveService = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!studioId) {
      toast({ title: "Error", description: "No se ha podido identificar el estudio. Por favor, recarga la página.", variant: "destructive"});
      return;
    }
    ...
    try {
        if (serviceData.name && serviceData.category && !isNaN(serviceData.duration) && !isNaN(serviceData.price)) {
            ...
            await addOrUpdateService(studioId, { id, ...serviceData });
            
            setOpen(false);
            setEditingService(null);
            await refreshData();
            toast({ title: "¡Éxito!", description: "El servicio se ha guardado correctamente." });
        } else {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (confirm('¿Estás seguro de que quieres eliminar este servicio?')) {
        try {
            await deleteService(studioId, id);
            await refreshData();
            toast({ title: "Servicio Eliminado", description: "El servicio se ha eliminado de tu lista." });
        } catch(e) {
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
                ...
                <Button disabled><PlusCircle className="mr-2 h-4 w-4" />Añadir Servicio</Button>
            </div>
            <Card>
                <CardHeader>
                  <CardTitle>Lista de Servicios</CardTitle>
                  <CardDescription>Gestiona tus servicios y sus detalles.</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="flex items-center justify-center py-10">
                        <Loader2 className="h-8 w-8 animate-spin text-muted-foreground" />
                        <p className="ml-4 text-muted-foreground">Cargando servicios...</p>
                    </div>
                </CardContent>
            </Card>
        </div>
            ...
            <DialogContent className="sm:max-w-[425px]">
                <form onSubmit={handleSaveService}>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {editingService ? "Actualiza los detalles del servicio." : "Completa los detalles del nuevo servicio."}
                        </DialogDescription>
                    </DialogHeader>
